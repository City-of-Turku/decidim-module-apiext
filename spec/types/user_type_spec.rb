    ...
    describe UserType, type: :graphql do
      ...
      describe "avatarUrl" do
        ...
        it "returns the default avatar url (small version)" do
          expect(response["avatarUrl"]).to include("//#{model.organization.host}:#{Capybara.server_port}#{::Webpacker.instance.manifest.lookup("media/images/default-avatar.svg")}")
        end
      end
